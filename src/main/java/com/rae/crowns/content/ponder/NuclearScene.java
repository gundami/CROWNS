...
public class NuclearScene {
    public static void reactor(SceneBuilder scene, SceneBuildingUtil sceneBuildingUtil) {
        ...
        scene.overlay.showSelectionWithText(sceneBuildingUtil.select.position(centerFuel),40).text("nuclear fuel naturally produce fast neutrons");
        scene.idle(50);
        scene.overlay.showSelectionWithText(sceneBuildingUtil.select.position(exteriorFuel),40).text("fast neutrons are unlikely to cause an other fuel block to undergo fission");
        scene.idle(50);
        ...
        scene.idleSeconds(5);
        scene.overlay.showSelectionWithText(sceneBuildingUtil.select.position(coal),40).text("70% efficiency for coal");
        scene.idleSeconds(2);
        scene.overlay.showSelectionWithText(sceneBuildingUtil.select.position(water),40).text("50% efficiency for water");
        scene.idleSeconds(2);
        scene.addKeyframe();
        ...
        scene.overlay.showSelectionWithText(bb, 80).text("when enough fuel are close to each other with a moderator");
        scene.idleSeconds(2);
        ...
        scene.overlay.showSelectionWithText(sceneBuildingUtil.select.layers(1,5),100).text("it's recommended to make reactors in a chest board manner");
        scene.idleSeconds(5);

        scene.overlay.showSelectionWithText(he,40).text("the higher the reactor, the more time the water will have to boil");
        scene.idleSeconds(3);
        scene.overlay.showSelectionWithText(sceneBuildingUtil.select.fromTo(5,3,3,5,6,3), 60).text("fuel becomes red when it's hotter than 3000 degrees Kelvin, it explodes at 3500");
